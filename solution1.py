...
for j in range(i_end + 1, i_start - 1):
    x0 = x[j]
    index = j
    ...
    for i in range(i_start - 1):
        if (abs(x[i] - x0) < 0.001):
            index_top = i
            break
    #вычисление площади верхней части методом площадей гаусса
    number_of_sides = index - index_top + 1
    s1 = 0
    for i in range(number_of_sides - 1):
        if (i == number_of_sides - 1):
            s1 += determ(x[index_top + i], u[index_top + i], x[index_top], u[index_top])
        s1 += determ(x[index_top + i], u[index_top + i], x[index_top + i + 1], u[index_top + i + 1])
    Square1 = 0.5 * abs(s1)
    #вычисление площади нижней части методом площадей гаусса
    number_of_sides = index_bottom - index + 1
    s2 = 0
    for i in range(number_of_sides - 1):
        if (i == number_of_sides - 1):
            s2 += determ(x[index + i], u[index + i], x[index], u[index])
        s2 += determ(x[index + i], u[index + i], x[index + i + 1], u[index + i + 1])
    Square2 = 0.5 * abs(s2)
    if (abs(Square1 - Square2) < 0.1):
        break
